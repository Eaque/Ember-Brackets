...
define(function (require, exports, module) {
    ...
    CodeMirror.defineMIME("text/x-brackets-html", {
         "name": "htmlmixed",
         "scriptTypes": [{"matches": /\/x-handlebars|\/x-mustache/i, "mode": null}]
     });
    ...
    CodeMirror.defineMode("mustache", function(config, parserConfig) {
      ...
      var mustacheOverlay = {
        token: function(stream, state) {
            if (stream.match("linkTo", false)) {
                for(i = 0 ; i < 6; i++) {
                        stream.next();
                    }
                    return "mustache-bold";
            }
    ...
    function setTheme() {
        ...
        if (currentDoc) {
            currentDoc._masterEditor._codeMirror.setOption("ember", name);
        }
    }
